...
class ContaoModelEntity extends AbstractModel implements Entity
{
     ...
     *
     * @param mixed $id Could be a integer, string or anything else - depends on the provider implementation.
     *
     * @return void
     */
    ...
    public function writeToPropertyValueBag(PropertyValueBagInterface $valueBag)
    {
        foreach ($this->getPropertiesAsArray() as $name => $value) {
            if ($valueBag->hasPropertyValue($name)) {
               $valueBag->setPropertyValue($name, $valueBag);
            }
        }
